...
exports.alkaneBumpFee = exports.alkaneGetAllPoolsDetails = exports.alkaneSimulate = exports.alkaneAddLiquidity = exports.alkaneCreatePool = exports.alkaneSend = exports.alkaneSplit = exports.alkaneSwap = exports.alkaneRemoveLiquidity = exports.alkaneExecute = exports.alkaneTokenDeploy = exports.alkaneContractDeploy = exports.alkanesTrace = void 0;
...
/* @dev example call
 oyl alkane bump-fee -txid "0x12345..." -feeRate 10

 提高现有Alkanes交易的手续费，创建一个替代交易
*/
exports.alkaneBumpFee = new commander_1.Command('bump-fee')
    .requiredOption('-txid, --txid <txid>', '要替换的交易ID')
    .requiredOption('-feeRate, --feeRate <feeRate>', '新的手续费率')
    .option('-m, --mnemonic <mnemonic>', '(optional) 用于签名交易的助记词 (default = TEST_WALLET)')
    .option('-p, --provider <provider>', '网络提供者类型 (regtest, bitcoin)')
    .action(async (options) => {
    const wallet = new wallet_1.Wallet(options);
    try {
        // 获取原始交易信息以便检查
        const txInfo = await wallet.provider.esplora.getTxInfo(options.txid);
        if (txInfo) {
            console.log(`交易信息: ${JSON.stringify(txInfo.vin[0], null, 2)}`);
            const hasRBF = txInfo.vin.some(input => input.sequence < 0xffffffff - 1);
            console.log(`是否启用RBF: ${hasRBF}, 序列号: ${txInfo.vin[0].sequence}`);
        }
        const result = await alkanes.bumpFee({
            txid: options.txid,
            newFeeRate: Number(options.feeRate),
            account: wallet.account,
            provider: wallet.provider,
            signer: wallet.signer,
        });
        ...
        console.error('提高手续费失败:', error.message);
    }
});
